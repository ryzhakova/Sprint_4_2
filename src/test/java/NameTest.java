import io.qameta.allure.junit4.DisplayName;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.junit.runners.Parameterized;

import static org.junit.Assert.assertEquals;

@RunWith(Parameterized.class)
@DisplayName("Проверка корректности имени для печати")
public class NameTest {
    private final String name;
    private final boolean expected;

    public NameTest(String name, boolean expected){
        this.name = name;
        this.expected = expected;
    }

    @Parameterized.Parameters(name="{index}. Input: {0}. Expected: {1}")
    public static Object[][] getName(){
        return new Object[][]{
                //валидное значение
                {"Тимоти Шаламе", true},
                // имя из двух символов
                {"Ти", false},
                // имя из трех символов (с проблелом) - минимальное значение
                {"Т м", true},
                // имя из 19 символов (с пробелом) - максимальное значение
                {"Бенедикт Камбербэтч", true},
                // имя из 20 символов
                {"Тимоти Шаламеблекста", false},
                //пробел перед именем
                {" Тимоти Шаламе", false},
                // пробел после имени
                {"Тимоти Шаламе ", false},
                // без пробелов
                {"ТимотиШаламе", false},
                // два пробела в центре
                {"Ти моти Шаламе", false},
                // пустая строка
                {"", false},
                // просто пробелы
                {"    ", false},
                // null
                {null, false},
        };
    }

    @Test
    @DisplayName("Проверка имени")
    public void checkNameTest(){
        Account account = new Account(name);
        assertEquals(expected, account.checkNameToEmboss());
    }
}
